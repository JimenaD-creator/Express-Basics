...
const activeMachineImage = {
  ...
  expressions: './img/machine-active.svg',
  animals: './img/server-machine-animals.svg'
};

const activeSingularRouteImage = {
  expressions: './img/expression-route-active.svg',
  animals: './img/animal-route-active.svg'
};

const activePluralRouteImage = {
  expressions: './img/expressions-route-active.svg',
  animals: './img/animals-route-active.svg'
};
  ...
  $.ajax('/', {
    success: function() {
      ...
      if ($('#expression-route').attr('src') === activeSingularRouteImage[getInactiveMode()] ||
           $('#expression-route').attr('src') === "./img/expression-route-inactive.svg") {
    ...
    error: function(xhr) {
      if (xhr.status !== 404) {
        ...
        $('#expression-route').attr('src', './img/expression-route-inactive.svg');
        $('#expressions-route').attr('src', './img/expressions-route-inactive.svg');
      }
    }
  });
}
...
function activateMachine() {
  ...
  $('#beaker').attr('src', './img/beaker-active-neutral.svg');

  if ($('#lightbulb').attr('src') === './img/lightbulb-inactive.svg') {
    $('#lightbulb').attr('src', './img/lightbulb-active.svg');
  }
}
...
function deactivateMachine() {
  ...
  $('#server-machine').attr('src', './img/server-machine-inactive.svg');
  $('#lightbulb').attr('src', './img/lightbulb-inactive.svg');
  $('#beaker').attr('src', './img/beaker-inactive.svg');
  $('#code-number').text('');
...
function triggerExpressionRequest() {
  ...
  $('#expression-route').attr('src', './img/expression-route-pending.svg');
  $('#expressions-route').attr('src', activePluralRouteImage[currentMode]);
  $('#code-number').text('');
  $('#code-description').text('');
  clearExpressionsText();
...
function triggerExpressionsRequest() {
  ...
  $('#expression-route').attr('src', activeSingularRouteImage[currentMode]);
  $('#expressions-route').attr('src', './img/expressions-route-pending.svg');
  $('#code-number').text('');
  $('#code-description').text('');
  clearExpressionText();
...
function animateMachine(animationDelay) {
  $('#beaker').attr('src', './img/beaker-pending.svg');
  $('#lightbulb').attr('src', './img/lightbulb-pending.svg');
  $('#sliders')
      .queue(function(next) { $(this).attr('src', './img/sliders-active-1.svg'); next(); })
      .delay(animationDelay)
      .queue(function(next) { $(this).attr('src', './img/sliders-active-2.svg'); next(); })
      .delay(animationDelay)
      .queue(function(next) { $(this).attr('src', './img/sliders-active-1.svg'); next(); })
      .delay(animationDelay)
      .queue(function(next) { $(this).attr('src', './img/sliders-active-2.svg'); next(); })
      .delay(animationDelay)
      .queue(function(next) { $(this).attr('src', './img/sliders-inactive.svg'); next(); });
  $('#buttons')
      .queue(function(next) { $(this).attr('src', './img/buttons-active-1.svg'); next(); })
      .delay(animationDelay)
      .queue(function(next) { $(this).attr('src', './img/buttons-active-2.svg'); next(); })
      .delay(animationDelay)
      .queue(function(next) { $(this).attr('src', './img/buttons-active-1.svg'); next(); })
      .delay(animationDelay)
      .queue(function(next) { $(this).attr('src', './img/buttons-active-2.svg'); next(); })
      .delay(animationDelay)
      .queue(function(next) { $(this).attr('src', './img/buttons-inactive.svg'); next(); });
}
...
function animateGoodExpressionRequest(statusCode, statusDescription, response) {
  ...
  $('#expression-route').attr('src', './img/expression-route-green.svg');
...
function animateGoodExpressionsRequest(statusCode, statusDescription, response) {
  animateGoodRequest(statusCode, statusDescription);
  $('#expressions-route').attr('src', './img/expressions-route-green.svg');
...
function animateGoodRequest(statusCode, statusDescription) {
  $('#lightbulb').attr('src', './img/lightbulb-green.svg');
...
function animateBadExpressionRequest(statusCode, statusDescription) {
  ...
  $('#expression-route').attr('src', './img/expression-route-red.svg');
}

function animateBadExpressionsRequest(statusCode, statusDescription) {
  animateBadRequest(statusCode, statusDescription);
  $('#expressions-route').attr('src', './img/expressions-route-red.svg');
}

function animateBadRequest(statusCode, statusDescription) {
  $('#lightbulb').attr('src', './img/lightbulb-red.svg');
